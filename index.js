...
// ═══════════════════════════════════════════════════════
// 🚀 SUKUNA PLUGIN SYSTEM INTEGRATION | نظام بلجنات سوكونا
// ═══════════════════════════════════════════════════════
const { pathToFileURL } = require('url');
const pluginFolder = path.join(__dirname, 'plugins');
global.plugins = {};

async function loadSukunaPlugins() {
  ...
  // دالة لقراءة كل الملفات حتى في المجلدات الفرعية
  const getFiles = (dir, fileList = []) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        else if (entry.endsWith('.js') || entry.endsWith('.mjs')) fileList.push(fullPath);
      } catch (e) {}
    }
    return fileList;
  };
  ...
  console.log('\n[Plugins] 🔄 Loading Sukuna Plugins into ZESHOO...');
  for (const filePath of files) {
    const relativePath = path.relative(pluginFolder, filePath).replace(/\\/g, '/');
    try {
      // استخدام import الديناميكي لدعم صيغة ES Modules بتاعة سوكونا
      const module = await import(`${pathToFileURL(filePath).href}?update=${Date.now()}`);
      global.plugins[relativePath] = module.default || module;
      loaded++;
      console.log(`[Plugins] ✅ ${relativePath}`);
    } catch (e) {
      failed++;
      console.error(`[Plugins] ❌ ${relativePath} | ${e.message.slice(0, 60)}`);
    }
  }
  console.log(`[Plugins] ✨ Done. Loaded: ${loaded}, Failed: ${failed}\n`);
}
...
async function runPlugins(sock, msg, session) {
  ...
  let [command, ...args] = noPrefix.split(/\s+/).filter(Boolean);
  ...
  if (isGroup) {
    try {
      ...
      if (senderP && (senderP.admin === 'admin' || senderP.admin === 'superadmin')) isAdmin = true;
      const botP = participants.find(p => p.id === botNumber);
      if (botP && (botP.admin === 'admin' || botP.admin === 'superadmin')) isBotAdmin = true;
    } catch (e) {}
  }
  ...
  for (const [name, plugin] of Object.entries(global.plugins)) {
    ...
    const isAccept = plugin.command instanceof RegExp
      ...
      : Array.isArray(plugin.command)
        ? plugin.command.some(cmd => cmd instanceof RegExp ? cmd.test(command) : String(cmd).toLowerCase() === command)
    ...
    // 🛡️ فحص الصلاحيات (Permissions)
    if (plugin.owner && !isOwner) return sock.sendMessage(from, { text: '⛔ هذا الأمر للمطور فقط.' }, { quoted: msg });
    if (plugin.admin && !isAdmin) return sock.sendMessage(from, { text: '⛔ هذا الأمر للمشرفين فقط.' }, { quoted: msg });
    if (plugin.botAdmin && !isBotAdmin) return sock.sendMessage(from, { text: '⛔ البوت يجب أن يكون مشرفاً.' }, { quoted: msg });
    if (plugin.group && !isGroup) return sock.sendMessage(from, { text: '⛔ للمجموعات فقط.' }, { quoted: msg });
    if (plugin.private && isGroup) return sock.sendMessage(from, { text: '⛔ للخاص فقط.' }, { quoted: msg });

    const extra = {
      conn: sock, sock, usedPrefix, noPrefix, args, text: args.join(' '), command,
      isOwner, isAdmin, isBotAdmin, isGroup, groupMetadata, session
    };

    // 🗣️ دالة الرد السريعة (m.reply)
    const reply = async (content, options = {}) => {
      if (typeof content === 'string') return sock.sendMessage(from, { text: content }, { quoted: msg, ...options });
      return sock.sendMessage(from, content, { quoted: msg, ...options });
    };

    try {
      const mockM = { ...msg, reply, sender, chat: from, text, isGroup, isOwner, isAdmin, isBotAdmin, name: msg.pushName || 'User' };
      if (typeof plugin === 'function') await plugin.call(sock, mockM, extra);
      else if (typeof plugin.run === 'function') await plugin.run.call(sock, mockM, extra);
    } catch (e) {
      console.error(`[Plugin Error] ${name}:`, e);
      await sock.sendMessage(from, { text: `❌ خطأ في تنفيذ الأمر: ${e.message}` }, { quoted: msg }).catch(() => {});
    }
    break; // تم تنفيذ الأمر، نخرج من اللوب
  }
}
